...
"""
from torch_utils.engine_kd import (
...
def main(args):
    ...
    if args['weights'] is None:
        ...
        model_student = build_model(num_classes=NUM_CLASSES, pretrained=False)

    # Load the teacher model.
    checkpoint = torch.load('/ssd1/sseth/codes/paro/20231016_Pretraining_Faster_RCNN_ViT_Detection_Model_on_Pascal_VOC/fasterrcnn-pytorch-training-pipeline/outputs/training/fasterrcnn_vitdet_voc/last_model_18.pth', map_location=DEVICE) 
    ...
    build_model = create_model['fasterrcnn_vitdet']
    model_teacher = build_model(num_classes=old_classes)
    # Load weights.
    model_teacher.load_state_dict(ckpt_state_dict)
    ...
    if args['weights'] is not None:
        ...
        # Build the new model with number of classes same as checkpoint.
        build_model = create_model[args['model']]
        model_student = build_model(num_classes=old_classes)
        # Load weights.
        model_student.load_state_dict(ckpt_state_dict)

        # Change output features for class predictor and box predictor
        # according to current dataset classes.
        in_features = model_student.roi_heads.box_predictor.cls_score.in_features
        model_student.roi_heads.box_predictor.cls_score = torch.nn.Linear(
            in_features=in_features, out_features=NUM_CLASSES, bias=True
        )
        model_student.roi_heads.box_predictor.bbox_pred = torch.nn.Linear(
    ...
    # Make the model_student transform's `min_size` same as `imgsz` argument. 
    model_student.transform.min_size = (args['imgsz'], )
    model_student = model_student.to(DEVICE)
    if args['sync_bn'] and args['distributed']:
        model_student = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_student)
    if args['distributed']:
        model_student = torch.nn.parallel.DistributedDataParallel(
            model_student, device_ids=[args['gpu']]
        )
    try:
        torchinfo.summary(
            model_student, 
    ...
    total_trainable_params = sum(
        p.numel() for p in model_student.parameters() if p.requires_grad)
    print(f"{total_trainable_params:,} training parameters.")
    # Get the model_student parameters.
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        _, batch_loss_list, \
            ...
            batch_loss_rpn_list, \
            batch_loss_kd_list = train_one_epoch(
            model_teacher,
            model_student, 
            optimizer, 
            train_loader, 
        ...
        loss_kd_list.append(np.mean(np.array(batch_loss_kd_list)))
        ...
        save_model_state(model_student, OUT_DIR, data_configs, args['model'])
